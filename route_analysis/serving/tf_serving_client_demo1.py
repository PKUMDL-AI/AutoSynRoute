...
flags = tf.flags
FLAGS = flags.FLAGS
tmp_string="NCCNS(=O)(=O)c1ccc(NC(=S)NC2CCCC2)cc1\nNCCNS(=O)(=O)c1ccc(N=C=S)cc1\nNCCNS(=O)(=O)c1ccc(N)cc1\nCC(=O)Nc1ccc(S(=O)(=O)NCCN)cc1"
...
flags.DEFINE_string("inputs_once", tmp_string, "Query once with this input.")

# flags.DEFINE_string("inputs_once", "O=C1CCC(N2Cc3c(cccc3[N+](=O)[O-])C2=O)C(=O)N1", "Query once with this input.")

flags.DEFINE_integer("timeout_secs", 500 , "Timeout for query.")
...
#   # flags.mark_flags_as_required(["problem", "data_dir"])
    ...
    # processes=[]
    # for input in inputs:
    #     p=Process(target=main, args=(input,))
    #     time.sleep(1)
    #     processes.append(p)
    #     p.start()
    # #
