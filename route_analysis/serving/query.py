...
def main(_):
  ...
  while True:
    ...
    outputs = serving_utils.predict(input_list, problem, request_fn)
    print(outputs)
#     outputs = np.array(outputs)
#     # output, score = outputs
#     output,score= outputs[:,0],outputs[:,1].astype('float')
#
#     print_str = """
# Input:
# {inputs}
...
def predict(input_class_list):
    ...
    problem.get_hparams(hparams)
    request_fn = make_request_fn()
    outputs = serving_utils.predict(input_class_list, problem, request_fn)
    return outputs
