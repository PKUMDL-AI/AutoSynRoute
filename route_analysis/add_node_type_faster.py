...
def expanded_node(state, opt):
    ...
    #
    # for i in range(40):
    #     next_probas = np.random.multinomial(1, node_preds, 1)
    #     next_int = np.argmax(next_probas)
    #     all_nodes.append(smi_nodes[next_int])
    # ix_pred=np.argsort(-node_preds)
...
def node_simulation(added_node, node_id, state, source_smi, workdir, iter_num):
    """------------------------------------------------------------------"""
    """rollout step"""
...
def chemreact_kn_simulation(state, added_nodes, opt):
    ...
    zipped_list = [i for i in zip(added_nodes, node_id_list, [state] * num_nodes, [source_smi] * num_nodes,
                                  [opt.input_dir] * num_nodes, [opt.iter_num] * num_nodes)]
    pool = Pool(20)
